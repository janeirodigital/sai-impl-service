...
import { WebHooksHandler, AuthenticatedAuthnContext, encodeWebId, IDelegatedGrantsJobData, IPushNotificationsJobData } from '../../../src';
...
test('should add push grants job if from reciprocal registration', (done) => {
  ...
  webHooksHandler.handle(ctx).subscribe({
    next: (response) => {
      expect(response.status).toBe(200)
      expect(grantsQueue.add).toBeCalledWith({ webId: aliceWebId, registeredAgent: bobWebId } as IDelegatedGrantsJobData)
