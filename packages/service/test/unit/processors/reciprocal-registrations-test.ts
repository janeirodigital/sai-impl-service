...
test('tries to discover reciprocial registration if not known', async () => {
  ...
  await expect(processor.processorFunction(job)).rejects.toThrow(`reciprocal registration from ${peerWebId} was not found`)
...
describe('webhook subscription', () => {
  ...
  test('subscribes and stores the subscription', async() => {
    ...
    await processor.processorFunction(job)
    expect(mockedSubscribe).toBeCalledWith(mockedRegistrationWithReciprocial.reciprocalRegistration.iri, webhookTargetUrl(webId, peerWebId), { fetch: saiSessionMock.rawFetch })
