...
export const addSocialAgent = async (saiSession: AuthorizationAgent, data: { webId: IRI, label: string, note?: string}): Promise<SocialAgent> => {
  ...
  const registration = await saiSession.registrySet.hasAgentRegistry.addSocialAgentRegistration(data.webId, data.label, data.note)
